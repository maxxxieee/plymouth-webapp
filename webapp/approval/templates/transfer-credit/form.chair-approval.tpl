...
<form action='{$PHP.BASE_URL}/transfer-credit/submit/advisor-approval' method="POST" class="{$form->classes()}">
	{box title = "Chair Form Approval" size = "8"}
	<ul>
		{$form->acceptable->as_li()}
		{$form->level->as_li()}
		{$form->equivalent->as_li()}
		{$form->satisfies_major_req->as_li()}
		{$form->satisfies_major_elect->as_li()}
		{$form->satisfies_minor_req->as_li()}
		{$form->satisfies_minor_elect->as_li()}
	</ul>
	{/box}
